...
### Git-дисциплина (репозиторий общий, два разработчика через Claude Code)
Репозиторий: `github.com/SavvaSupchik/youtube-auto-studio` (приватный).
Над проектом работают два человека через Claude Code — без дисциплины легко
разъехаться или затереть чужие изменения.

- **Перед началом работы** — `git pull`, чтобы подтянуть чужие изменения
  до того как начинать редактировать (репозиторий также синхронизируется
  через Google Диск между компьютерами — см. README, риск конфликтов выше
  обычного).
- **После завершения каждой задачи** — закоммитить и запушить в GitHub
  (`git add` конкретных файлов, `git commit`, `git push origin main`),
  **не дожидаясь отдельной просьбы пользователя**. Это явное разрешение
  автономно коммитить/пушить в рамках ЭТОГО проекта — переопределяет
  дефолтное поведение "коммитить только по явной просьбе".
- Коммитить осмысленными порциями с понятным сообщением (что и зачем),
  не одной гигантской свалкой правок.
- Не использовать `git add -A`/`git add .` вслепую — добавлять файлы по
  имени, чтобы случайно не закоммитить `.env`, `data/` или другой мусор
  (всё это и так в `.gitignore`, но двойная проверка не помешает).
- Если `git push` отклонён (в remote появились чужие коммиты) — сделать
  `git pull` (слияние/rebase), разрешить конфликты руками, и только потом
  запушить снова. **Никогда не `push --force` в `main`.**
- Секреты (`.env`, ключи API) никогда не коммитить — только через `.env`,
  каждый разработчик держит свой локально.
